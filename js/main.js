	...
	if($megaMenu.length) {
		...
		if (windowWidth > 991) {
			$megaMenu.each(function(){
				...
				$thisMegaMenu.css({'display': 'block'});
				var thisWidth = $thisMegaMenu.outerWidth();
			
				
				$thisMegaMenu.css({ 'display': 'none'});
...
function windowLoadInit() {
	...
	jQuery('form.contact-form').on('submit', function( e ){
		...
		var ajax = jQuery.post( "mailto.php", request )
		.done(function( data ) {
		...
		.fail(function( data ) {
			jQuery($form).find('[type="submit"]').attr('disabled', false).parent().append('<span class="contact-form-respond highlight">Mail cannot be sent. Our Server Is Busy Now.</span>');
		})
	});
